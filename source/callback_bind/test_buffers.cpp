...
void ext_test_buffers(pybind11::module_ & m) {
...
#undef PYBIND11_ASSIGN_HELPER
    ...
    public:
        ...
        Matrix(py::ssize_t rows, py::ssize_t cols, float *data) : m_rows(rows), m_cols(cols) {
            print_move_assigned(this, std::to_string(m_rows) + "x" + std::to_string(m_cols) + " matrix");
            // NOLINTNEXTLINE(cppcoreguidelines-prefer-member-initializer)
            m_data = data;// new float[(size_t)(rows * cols)];
            m_shallow = true;
            //memset(m_data, 0, sizeof(float) * (size_t)(rows * cols));
        }
    ...
    py::class_<Matrix>(m, "Matrix", py::buffer_protocol())
        ...
        .def(py::init([](const py::buffer &b) {
            ...
            auto *v = new Matrix(info.shape[0], info.shape[1], (float*)info.ptr);
